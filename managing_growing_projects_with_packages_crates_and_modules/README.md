...
These features, sometime collectively referred to as the _module system_, include:
...
Unfortunately, the code in Listing 7-5 still result in an error, as shown in Listing 7-6.
...
But the _contents_ of `hosting` are still private; making the module public doesn't make its contents public.
...
Listing 7-7: Adding the `pub` keyword to `mod hosting` and `fn add_to_waitlist` lets us cal the function from `eat_at_restaurant`
...
While `front_of_house` isn't public, because the `eat_at_restaurant` function is defined in the same module as `front_of_house` (that is, `eat_at_restaurant` and `front_of_house` are sibling), we can refer to `front_of_house` from `eat_at_restaurant`.
...
In the relative path, the logic is the same as the absolute path expect for the first step: rather than starting from the crate root, the path starts from `front_of_house`.
...
If you plan on sharing your library crate so other projects can use you code, your public API is your contract with users of your crate that determines how they can interact with your code.
...
The `fix_incorrect_order` function is in the `back_of_house` module, so we can use `super` to go to the parent module of `back_of_house`, which in this case in `crate`, the root.
...
Therefore, we used `super` so we'll have fewer place to update code in the feature if this code gets moved to a different module.
...
pub fn eat_at_restaurant() {
    ...
    // too se or modify the seasonal fruit that comes with the meal
...
If `Breakfast` didn't have such a function, we couldn't create an instance of `Breakfast` in `eat_at_restaurant` because we couldn't set the value of private `seasonal_fruit` field in `eat_at_restaurant`.

In construct, if we make an enum public, all of its variants are then public.
...
Enums aren't very useful unless their variants are public; it would be annoying to have an annotate all enum variants with `pub` in every case, so the default for enum variants is to be public.
Structs are often useful without thier fields being public, so struct fields follow the general rule of everything being private by default unless annotated with `pub`.
...
Having to write out the paths to call function can fell inconvenient and repetitive.
In Listing 7-7, whether we choose the absolute or relative path to the `add_to_waitlist` function, every time we wanted to call `add_to_waitlist` we had to specify `front_of_house` and `hosting` too.
Fortunately, there's a way to simplify this process: we can create shortcut to a path with the `use` keyword once, and then use the shorter name everywhere else in the scope.

In Listing 7-11, we bring the `crate::front_of_house::hosting` module into the scope of the `eat_at_restaurant` function so we only have to specify `hosting::add_to_waitlist` to all the `add_to_waitlist` function in `eat_at_restaurant`.
...
Listing 7-12 moves the `eat_at_restaurant` function into a new child module named `customer`, which is them a different scope than the `use` statement, so the function body won't compile:
...
Notice there's also warning that the `use` is no longer used in its scope!
...
In Listing 7-11, you might have wondered why we specified `use crate::front_of_house::hosting` and then called `hosting::add_to_waitlist` function to achieve the same result, as in Listing 7-13.
...
On the order hand, when bringing in struts, enums, and other items with `use`, it's idiomatic to specify the full path.
Listing 7-14 shows the idiomatic way to bring the standard library's `HashMap` strut into the scope of a binary crate.
...
As you can see, using the parent module distinguishes the two `Result` types.
If instead we specified `use std::fmt::Result` and `use std::io::Result`, we'd have to `Result` types in the same scope and Rust wouldn't know which one we meant when we used `Result`.

## Providing New Names with the `as` Keyword

There's another solution to the problem of bringing two types of the same name into the same scope with `use`: after the path, we can specify `as` add a new local name, or _alias_, for the type.
Listing 7-16 shows another way to write the code in Listing 7-15 by renaming one of the two `Resuult` types using `as`.
...
In the second `use` statement, we can chose the new name `IoResult` for the `std::io::Result` type, which won't conflict with the `Result` from `std::fmt` that we've also brought into scope.
...
For example, these two `use` statements we had in the Guessing in Listing 2-4 bring items form `std` into scope:
...
Instead, we can use nested path, followed by two colons, and then curly brackets around a list of the parts of the paths that differ, as shown in Listing 7-18.
...
In bigger programs, bringing many items into scope from the same crate or module using nested paths can reduce the number of separate `use` statements needed by a log!

We can use a nested path at any level in path, which is useful when combining two `use` statements that share subpath.
...
Listing 7-20: Combining the paths in Listing 7-19 into one `use` statements.
