...
This code will print each pair in arbitrary order.
...
Another common use case for hash map is to look up a key's value and then update it based on the old value.
For instance, Listing 8-25 shows code that counts how many times each word appears in some text.
We use a hash map with the word as keys and increment the value to keep track of how many times we've seen that word.
...
Listing 8-25: Counting occurrences of words using a hash map that stores word and counts.

This code will print `{"word": 2, "hello": 1, "wonderful": 1}`.
You might see the same key/value pairs printed in a different order: recall from the ["Accessing Values in a Hash Map"](https://doc.rust-lang.org/book/ch08-03-hash-maps.html#accessing-values-in-a-hash-map) section that iterating over a hash map happens in an arbitrary order.
...
By default, `HashMap` uses a hashing function called _SipHash_ that can provide resistance to Denial of Service (DoS) attacks involving hash tables [¹](https://doc.rust-lang.org/book/ch08-03-hash-maps.html#siphash).
